    ...
    <Card className="flex h-full min-h-0 flex-col overflow-hidden border-border/70 lg:sticky lg:top-4">
      <CardHeader className="gap-3 border-b bg-gradient-to-b from-muted/30 to-background pb-4">
        <div className="space-y-1">
          <CardTitle className="text-base">{i18next.t("novels.chapterExecutionQueueCard.g1rksa")}</CardTitle>
          <p className="text-sm leading-6 text-muted-foreground">{i18next.t("novels.chapterExecutionQueueCard.npeivw")}</p>
        </div>
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col pb-4 pt-4">
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
          {chapters.length === 0 ? (
            <div className="rounded-xl border border-dashed p-4 text-xs leading-6 text-muted-foreground">{i18next.t("novels.chapterExecutionQueueCard.6k5aub")}</div>
          ) : (
                ...
                >
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      ...
                      {isRepairTarget ? (
                        <Badge variant="secondary" className="rounded-full px-2 py-1 text-[11px]">{i18next.t("novels.chapterExecutionQueueCard.c51ke")}</Badge>
                      ) : null}
