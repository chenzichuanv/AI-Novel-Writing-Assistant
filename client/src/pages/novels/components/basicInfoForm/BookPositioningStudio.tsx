...
const POSITIONING_FIELDS = [
  { key: "title", label: i18next.t("dict.gen_32c65d8d") },
  { key: "description", label: i18next.t("novels.bookPositioningStudio.iqi6") },
  { key: "targetAudience", label: i18next.t("novels.bookPositioningStudio.oiga") },
  { key: "bookSellingPoint", label: i18next.t("novels.bookPositioningStudio.eskj") },
  { key: "first30ChapterPromise", label: i18next.t("novels.bookPositioningStudio.vt38pc") },
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className="border-primary/25 bg-primary/5 text-primary">{i18next.t("novels.bookPositioningStudio.af7hd0")}</Badge>
                <span className="text-xs font-medium text-muted-foreground">{i18next.t("novels.bookPositioningStudio.vzk46r")}</span>
              </div>
              <h2 className="mt-3 text-lg font-semibold leading-7 text-foreground">{i18next.t("novels.bookPositioningStudio.7es0jb")}</h2>
              <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">{i18next.t("novels.bookPositioningStudio.lo6zwo")}</p>
            </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,0.85fr),minmax(20rem,1.15fr)]">
            <div className="space-y-3 rounded-lg border border-border/60 bg-muted/10 p-3">
              <div className="space-y-2">
                <FieldLabel htmlFor="basic-title">{i18next.t("basicInfo.novelTitle")}</FieldLabel>
                <Input
                  id="basic-title"
                  value={basicForm.title}
                  placeholder={i18next.t("basicInfo.novelTitlePlaceholder")}
              ...
              <div className="space-y-2">
                <FieldLabel htmlFor="basic-description">{i18next.t("basicInfo.leadSummary")}</FieldLabel>
                <textarea
                  ...
                  placeholder={i18next.t("basicInfo.leadSummaryPlaceholder")}
          ...
          <div className="space-y-5">
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-foreground">{i18next.t("novels.bookPositioningStudio.de5qhb")}</div>
            ...
            <div className="space-y-3 rounded-lg border border-border/60 bg-background/85 p-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <Sparkles className="h-4 w-4 text-primary" />{i18next.t("novels.bookPositioningStudio.b356uj")}</div>
              {commercialTags.length > 0 ? (
                ...
                <p className="text-sm leading-6 text-muted-foreground">{i18next.t("novels.bookPositioningStudio.l2z6m9")}</p>
              )}
            </div>
            ...
            <div className="grid gap-2 text-xs text-muted-foreground">
              <div className="flex items-start gap-2 rounded-md bg-background/70 p-2">
                ...
                <span>{i18next.t("novels.bookPositioningStudio.2c1zm7")}</span>
              </div>
              <div className="flex items-start gap-2 rounded-md bg-background/70 p-2">
                <Map className="mt-0.5 h-3.5 w-3.5 text-sky-600" />
                <span>{i18next.t("novels.bookPositioningStudio.jzruhg")}</span>
              </div>
              <div className="flex items-start gap-2 rounded-md bg-background/70 p-2">
                <FileText className="mt-0.5 h-3.5 w-3.5 text-amber-600" />
                <span>{i18next.t("novels.bookPositioningStudio.5cyxnu")}</span>
              </div>
            </div>
            ...
            {coverSection ? (
              <div className="rounded-lg border border-border/60 bg-background/85 px-3 pb-3 [&>section]:border-t-0 [&>section]:pt-3">
                <div className="flex items-center gap-2 pt-3 text-sm font-semibold text-foreground">
                  <Image className="h-4 w-4 text-primary" />{i18next.t("novels.bookPositioningStudio.c8llhg")}</div>
                {coverSection}
              </div>
            ) : null}
          </div>
